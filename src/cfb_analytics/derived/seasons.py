...
SEASON_SCHEMA_VERSION="team-season-v6-turnovers"
...
def derive_team_seasons(team_games,season):
 ...
 for team,rows in sorted(grouped.items()):
  ...
  for version in ("successDefinitionVersion","explosivenessDefinitionVersion","finishingDrivesDefinitionVersion","fieldPositionDefinitionVersion","turnoversDefinitionVersion"):row[version]=next((r.get(version) for r in rows if r.get(version)),None)
  out.append(row)
 return out
