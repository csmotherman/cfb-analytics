...
PREGAME_VERSION="cfb-sandbox-pregame-v2-components"
MATCHUP_VERSION="cfb-sandbox-matchups-v2-components"
...
def build_pregame(raw_root:Path,processed_root:Path,season:int,refresh_components=False):
 comp=materialize_components(raw_root,processed_root,season,refresh_components);components=comp["rows"]
...
def materialize_sandbox_pregame(raw_root:Path,processed_root:Path,season:int,refresh=False):
 ...
 if not refresh and snap_path.exists() and match_path.exists() and manifest_path.exists():
  ...
  if m.get("sourceSignature")==sig and m.get("cacheVersion")==CACHE_VERSION and m.get("componentVersion")==COMPONENT_VERSION and m.get("sandboxSystemsVersion")==SANDBOX_SYSTEMS_VERSION and m.get("pregameVersion")==PREGAME_VERSION and m.get("matchupVersion")==MATCHUP_VERSION:return {"cache_status":"REUSED","component_status":"REUSED","snapshots":json.loads(snap_path.read_text()),"matchups":json.loads(match_path.read_text())}
