...
def build_pregame(raw_root:Path,processed_root:Path,season:int):
 ...
 for st,w in sorted(discover_partitions(raw_root,season),key=lambda x:_pk(*x)):
  plays,drives,games=_load(processed_root,season,st,w)
  ratings={r["Team"]:r for r in compute_systems(hp,hd)} if hd else {}
  for g in games:
   t=g.get("team");r=ratings.get(t);row={"season":season,"seasonType":st,"week":w,"gameId":str(g.get("gameId")),"team":t,"opponent":g.get("opponent"),"gamesPlayedBefore":games_before[t],"historyAvailable":r is not None,"sandboxSystemsVersion":SANDBOX_SYSTEMS_VERSION,"sandboxPregameVersion":PREGAME_VERSION}
   for s in SYSTEMS:
    row[f"{s}_Off"]=r.get(f"{s}_Off") if r else None;row[f"{s}_Def"]=r.get(f"{s}_Def") if r else None
   out.append(row)
  for g in games:games_before[g.get("team")]+=1
  hp.extend(plays);hd.extend(drives)
 return out
...
def build_matchups(snaps,season):
 ...
 for gid,rows in sorted(by.items()):
  ...
  for s in SYSTEMS:
   ...
   r[f"team1_{s}_OffenseEdge"]=float(ao)-float(bd) if _num(ao) and _num(bd) else None
   r[f"team1_{s}_DefenseEdge"]=float(ad)-float(bo) if _num(ad) and _num(bo) else None
   r[f"team2_{s}_OffenseEdge"]=float(bo)-float(ad) if _num(bo) and _num(ad) else None
   r[f"team2_{s}_DefenseEdge"]=float(bd)-float(ao) if _num(bd) and _num(ao) else None
  out.append(r)
 return out
...
def audit_matchups(snaps,rows):
 ...
 for r in rows:
  for s in SYSTEMS:
   a,b=r.get(f"team1_Off_{s}"),r.get(f"team2_Def_{s}");e=r.get(f"team1_{s}_OffenseEdge")
   edge_ok=edge_ok and ((e is None) if not (_num(a) and _num(b)) else abs(e-(a-b))<1e-12)
