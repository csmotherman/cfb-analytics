...
GAME_SCHEMA_VERSION="team-game-v4-finishing-drives"
...
def derive_team_games(plays,drives,season,season_type,week):
 ...
 for gid,ds in by_game.items():
  ...
  finishing={team:team_finishing_metrics(team,ds,gp) for team in teams}
  for team in sorted(teams):
   ...
   row.update(_metric_fields([p for p in gp if p.get("offense")==team],[p for p in gp if p.get("defense")==team]));row.update(finishing[team]);row["finishingDrivesDefinitionVersion"]=FINISHING_DRIVES_VERSION
   if opponent in finishing:row.update(_allowed_finishing(finishing[opponent]))
   else:row.update(_allowed_finishing({}))
   out.append(row)
 return out
