...
GAME_SCHEMA_VERSION="team-game-v5-field-position"
...
def derive_team_games(plays,drives,season,season_type,week):
 ...
 for gid,ds in by_game.items():
  ...
  for team in sorted(teams):
   ...
   row.update(_metric_fields([p for p in gp if p.get("offense")==team],[p for p in gp if p.get("defense")==team]));row.update(finishing[team]);row["finishingDrivesDefinitionVersion"]=FINISHING_DRIVES_VERSION;row.update(fieldpos[team]);row["fieldPositionDefinitionVersion"]=FIELD_POSITION_VERSION
