...
FEATURE_STORE_VERSION = "model-feature-store-v1"
...
def materialize_feature_store(raw_root: Path, processed_root: Path, season: int, refresh: bool = False) -> dict:
    ...
    if not refresh and path.exists() and manifest_path.exists():
        ...
        if (
            ...
            return {"season": season, "status": "REUSED", "rows": manifest.get("recordCount", 0), "path": str(path)}
    ...
    return {"season": season, "status": "WRITTEN", "rows": len(rows), "path": str(path)}
...
def build_all(raw_root: Path, processed_root: Path, seasons=DEFAULT_SEASONS, refresh: bool = False, workers: int = 4) -> list[dict]:
    ...
    with ProcessPoolExecutor(max_workers=min(workers, len(jobs))) as pool:
        ...
        for future in as_completed(futures):
            ...
            out.append(result)
            print(f"FEATURE STORE {result['season']}: {result['status']} rows={result['rows']:,}", flush=True)
...
def main() -> None:
    ...
    if args.all:
        ...
        print(f"FEATURE STORE {result['season']}: {result['status']} rows={result['rows']:,}")
    else:
