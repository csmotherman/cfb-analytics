...
record semantics rather than turnover-anchor proximity:
  PASS_COMPLETION, PASS_INCOMPLETE, INTERCEPTION, SACK.
...
def evidence_class(p):
    ...
    # Order is deliberate: explicit interception/sack semantics outrank generic pass text.
...
def audit(plays,drives):
    ...
    for d in drives:
        ...
        evidence=[p for p in rows if evidence_class(p) in ("PASS_COMPLETION","PASS_INCOMPLETE","INTERCEPTION","SACK")]
...
def concise(r):
    ...
    return "\n".join([
      "DROPBACK v1 TAXONOMY FORENSICS",
      ...
      "",
      "Diagnostic only. Do not lock until residual PASS-text records and interception possessions without evidence are understood.",
      "Use --json for representative residual and multi-evidence sequences."
    ])
