"""Integrity audits for raw FBS-vs-FBS partitions and seasons."""
...
def audit_partition(root: Path, season: int, season_type: str, week: int) -> dict[str, Any]:
    ...
    checks = {
        "manifests_valid": all(verify_manifest(directory, e) for e in ENTITIES),
        "fbs_vs_fbs_only": not non_fbs_games,
        "game_partition_metadata_matches": not partition_mismatch,
    ...
    return {
        ...
        "checks": checks,
        "duplicates": {"games": len(game_ids)-len(game_set), "drives": len(drive_ids)-len(drive_set), "plays": len(play_ids)-len(play_set)},
        "coverage": {"games_with_drives": len(game_set & drive_game_set), "games_without_drives": len(game_set-drive_game_set), "games_with_plays": len(game_set & play_game_set), "games_without_plays": len(game_set-play_game_set), "drives_referenced_by_plays": len(drive_set & play_drive_set), "drives_without_plays": len(drive_set-play_drive_set)},
        "orphans": {"drive_game_ids_missing_from_games": sorted(drive_game_set-game_set), "play_game_ids_missing_from_games": sorted(play_game_set-game_set), "play_drive_ids_missing_from_drives": sorted(play_drive_set-drive_set)},
        "non_fbs_game_ids": non_fbs_games,
        "partition_metadata_mismatch_game_ids": partition_mismatch,
...
def discover_partitions(root: Path, season: int) -> list[tuple[str, int]]:
    season_dir = root / "cfbd" / f"season={season}"
    ...
    for type_dir in season_dir.glob("season_type=*"):
        ...
        for week_dir in type_dir.glob("week=*"):
            try:
                ...
                continue
            if all((week_dir / f"{e}.json").exists() for e in ENTITIES):
                found.append((season_type, week))
    return sorted(found, key=lambda x: (x[0], x[1]))


def audit_season(root: Path, season: int) -> dict[str, Any]:
    partitions = discover_partitions(root, season)
    audits = [audit_partition(root, season, st, wk) for st, wk in partitions]
    totals = Counter()
    all_ids: dict[str, list[str]] = defaultdict(list)
    schema_sets: dict[str, dict[str, list[str]]] = {e: {} for e in ENTITIES}
    for audit in audits:
        label = f"{audit['partition']['season_type']}:W{audit['partition']['week']:02d}"
        directory = partition_dir(root, season, audit['partition']['season_type'], audit['partition']['week'])
        for entity in ENTITIES:
            rows = _load(directory, entity)
            totals[entity] += len(rows)
            all_ids[entity].extend(str(r.get('id')) for r in rows)
            schema_sets[entity][label] = audit['schema'][entity]['fields']
    cross_duplicates = {e: len(ids)-len(set(ids)) for e, ids in all_ids.items()}
    schema_variants = {e: len({tuple(v) for v in by_partition.values()}) for e, by_partition in schema_sets.items()}
    checks = {
        "all_partitions_pass": bool(audits) and all(a['status'] == 'PASS' for a in audits),
        "no_cross_partition_duplicate_game_ids": cross_duplicates['games'] == 0,
        "no_cross_partition_duplicate_drive_ids": cross_duplicates['drives'] == 0,
        "no_cross_partition_duplicate_play_ids": cross_duplicates['plays'] == 0,
    }
    return {
        "season": season,
        "status": "PASS" if all(checks.values()) else "REVIEW",
        "partition_count": len(audits),
        "checks": checks,
        "totals": dict(totals),
        "cross_partition_duplicates": cross_duplicates,
        "schema_variant_counts": schema_variants,
        "schema_by_partition": schema_sets,
        "partitions": [{"season_type": a['partition']['season_type'], "week": a['partition']['week'], "status": a['status'], "counts": a['counts'], "coverage": a['coverage'], "failed_checks": [k for k,v in a['checks'].items() if not v]} for a in audits],
    }
