  ...
  return <div className="page-stack page-pad history-season-page">
    ...
    <div className="history-season-layout">
      <main className="history-season-main">
        ...
        {playoffGames.length > 0 && <section className="history-playoff-section"><span className="eyebrow">PLAYOFF RUN</span><div className="season-results playoff-results">{playoffGames.map((game) => <div className={`history-game-row playoff-game ${game.win ? "playoff-win" : "playoff-loss"} ${game.playoff?.round === "championship" ? "title-game" : ""}`} key={game.id}><b className={game.win ? "win" : "loss"}>{game.win ? "W" : "L"}</b><TeamLogo teamId={game.opponentId} name={game.opponent} size={64}/><span>{game.playoff?.roundName?.toUpperCase()}</span><strong>vs {game.opponent}</strong><em>{game.pointsFor ?? "—"}–{game.pointsAgainst ?? "—"}</em></div>)}</div></section>}

        <details className="history-disclosure"><summary><span>FULL SCHEDULE</span><b>{results.length} GAMES · {wins}–{losses}</b></summary><div className="season-results">{results.map((game) => <div className={`history-game-row ${game.playoff?.competition === "cfp" ? "all-games-playoff" : ""}`} key={game.id}><b className={game.win ? "win" : "loss"}>{game.win ? "W" : "L"}</b><TeamLogo teamId={game.opponentId} name={game.opponent} size={64}/><span>{game.playoff?.competition === "cfp" ? "CFP" : `WEEK ${game.week}`}</span><strong>{game.away ? "at " : "vs "}{game.opponent}</strong><em>{game.pointsFor ?? "—"}–{game.pointsAgainst ?? "—"}</em></div>)}</div></details>
      </main>
