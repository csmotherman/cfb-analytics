...
export default function More(){
  return <div className="more-page"><div className="wrap more-page-inner"><header className="more-page-header"><span>EXPLORE</span><h1>MORE MICHIGAN</h1><p>Everything beyond the main team, schedule, and analytics tabs.</p></header><div className="more-hub-grid">{sections.map(item=><Link href={item.href} key={item.href} className="more-hub-card"><small>{item.tag}</small><div><h2>{item.label}</h2><p>{item.description}</p></div><span aria-hidden="true">›</span></Link>)}</div></div></div>;
}
