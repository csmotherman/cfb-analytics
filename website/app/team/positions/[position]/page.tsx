...
import { currentRoster,playersByPosition } from "../../../../lib/michigan/roster";
type Props={params:Promise<{position:string}>};
export const dynamicParams=false;
export function generateStaticParams(){return Array.from(new Set(currentRoster().map(player=>player.position).filter((position):position is string=>Boolean(position)))).map(position=>({position:position.toLowerCase()}));}
