...
export default async function PlayerPage({params,searchParams}:Props){
  ...
  const SeasonTable=()=>hasStats?<div className="player-stat-table-wrap"><table className="player-stat-table"><thead><tr><th>YEAR</th><th>TEAM</th><th>POS</th>{statColumns.map(stat=><th key={statKey(stat)}>{statLabel(stat)}</th>)}</tr></thead><tbody>{careerSeasons.map(season=><tr key={`${season.season}-${season.team}`}><td className="player-stat-year">{season.season}</td><td><div className="player-stat-team">{season.teamId?<img src={teamLogoUrl(season.teamId,64)} alt=""/>:<span className="player-season-logo-fallback">M</span>}<span>{season.team}</span></div></td><td>{season.position??"—"}</td>{statColumns.map(column=>{const value=season.displayStats.find(stat=>statKey(stat)===statKey(column))?.value;return <td key={statKey(column)}>{value==null?"—":statValue(value)}</td>;})}</tr>)}</tbody></table></div>:<div className="player-focus-empty"><b>No verified college box-score production yet.</b><p>{freshman?"Recruiting information is the honest baseline until his first college season begins.":"No position-relevant public box-score statistics are currently published for this player."}</p></div>;
