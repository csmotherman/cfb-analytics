...
export function currentRoster(): MichiganPlayer[] {
  ...
  const productionGrades = readJson<Array<{playerId:string;grade?:MichiganPlayer["performanceGrade"];season:number;basis:string}>>("data", "published", "2026", "michigan", "player-production-grades.json") ?? [];
  ...
  return roster.map((player) => {
    const grade = byId.get(player.id); const production = productionById.get(player.id); const status = statusById.get(player.id);
