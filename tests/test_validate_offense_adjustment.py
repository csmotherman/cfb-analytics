...
from tests.test_least_squares_offense import _network_rows


def test_cross_validation_keeps_game_rows_together_and_scores_all_models():
    result = cross_validate(_network_rows(), 2025, folds=3, seed="unit-test")
...
def test_cross_validation_is_deterministic_for_same_seed():
    a = cross_validate(_network_rows(), 2025, folds=3, seed="same")
    b = cross_validate(_network_rows(), 2025, folds=3, seed="same")
    assert a == b
