...
from cfb_analytics.profiles.opponent_adjustment import fit_context
...
def paired_week(week, a_rush, a_pass, b_rush=8, b_pass=8):
    gid = f"g{week}"
    return [
        game(week, "A", a_rush, a_pass, opponent="B", game_id=gid),
        game(week, "B", b_rush, b_pass, opponent="A", game_id=gid),
    ]
...
def test_snapshots_capture_recent_state_separately_from_full_season():
    rows = []
    for w in range(1, 9):
        rows += paired_week(w, 8 if w <= 4 else 16, 8 if w <= 4 else 16)
...
def test_opponent_adjustment_separates_defenses_with_shared_offense_context():
    rows = []
    # A and C each face both X and Y. Y consistently allows less success than X,
    # so the schedule is connected and the additive offense/defense effects are
    # identifiable instead of confounding each defense with a unique offense.
    for w in range(1, 5):
        rows += [
            game(w, "A", 16, 16, opponent="X", game_id=f"ax{w}"),
            game(w, "A", 8, 8, opponent="Y", game_id=f"ay{w}"),
            game(w, "C", 14, 14, opponent="X", game_id=f"cx{w}"),
            game(w, "C", 6, 6, opponent="Y", game_id=f"cy{w}"),
        ]
...
def test_hierarchical_discovery_produces_more_than_broad_families():
    ...
    for i in range(360):
        ...
        for j, f in enumerate(fields):
            r[f] = max(1.0, min(99.0, base + ((j % 4) - 1.5) * 3 + (i % 3)))
