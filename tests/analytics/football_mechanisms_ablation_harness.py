...
from cfb_analytics.analytics.football_mechanisms import FAMILY_FEATURES,ALL_MATCHUP_FEATURES,orient_matchup
...
MECH=tuple(ALL_MATCHUP_FEATURES)
DERIVED=("srsXExpectedPossessions","mwdrXExpectedPossessions")
FULL=BASE+MWDR+MECH+DERIVED
...
def load_all():
 pr=Path("data/processed");data={};print("Loading saved feature stores only; no play-by-play will be recomputed.")
 for season in DEFAULT_SEASONS:
  base=load_saved_feature_store(pr,season);p=pr/"derived"/"football_mechanisms"/f"season={season}"/"matchups.json"
  if not p.exists():raise FileNotFoundError(f"Missing football-mechanisms store for {season}. Run: python -m cfb_analytics.analytics.football_mechanisms --all")
...
def main():
 data=load_all();models={"BASE_ITERATIVE_SRS":BASE,"BASE_PLUS_MWDR":BASE+MWDR}
 for name,features in FAMILY_FEATURES.items():models[f"BASE_PLUS_{name}"]=BASE+tuple(features)
 models["BASE_PLUS_ALL_MECHANISMS"]=BASE+MECH
 models["BASE_PLUS_SRS_POSSESSION_SCALE"]=BASE+("srsXExpectedPossessions",)
 models["BASE_PLUS_MWDR_POSSESSION_SCALE"]=BASE+MWDR+("mwdrXExpectedPossessions",)
 models["BASE_PLUS_MWDR_AND_ALL_MECHANISMS"]=BASE+MWDR+MECH
 models["BASE_PLUS_MWDR_ALL_MECHANISMS_AND_INTERACTIONS"]=BASE+MWDR+MECH+DERIVED
 print("FOOTBALL MECHANISMS v1 ABLATION")
 ...
 print("Families: "+", ".join(FAMILY_FEATURES))
 for min_games in (3,4):
  ...
  for test_season in TEST_SEASONS:
   ...
   full=scored["BASE_PLUS_ALL_MECHANISMS"]
   print("LEAVE-ONE-FAMILY-OUT FROM ALL_MECHANISMS:")
   for family,features in FAMILY_FEATURES.items():
    keep=tuple(k for k in MECH if k not in features);z=score(fit(stats,BASE+keep),test)
    print(f"  minus {family}: MAE {z['mae']-full['mae']:+.3f}, RMSE {z['rmse']-full['rmse']:+.3f}, Winner {(z['winner']-full['winner'])*100:+.2f} pp")
